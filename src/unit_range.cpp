...
cUnitRange::cUnitRange(cUnit &u) : unit(u){
	...
	m_actionRangeSprite = assetAccess.getSprite("Unit range component", "attack_range");
}
...
// Calculate attack range on board
void cUnitRange::calculateActionRange(cBoard &board, int minRange, int maxRange){
	// Set constants used for rangefinsing
	const int boardLine = board.getSize().x;
	const int positionTile = unit.m_pos.x + (unit.m_pos.y * boardLine);
	const int lastTile = board.getSize().x * boardLine - 1;

	// Reset attack range containers
	m_validAction.clear();
	m_validActionDistance.clear();

	// BFS algorithm, create queue, push current position
	std::queue<int> tilesToCheck;
	tilesToCheck.push(positionTile);
	m_validActionDistance[positionTile] = 0;

	// Search through tiles
	while(not tilesToCheck.empty()){
		const int CURRENT_DISTANCE = m_validActionDistance[tilesToCheck.front()];
		if(CURRENT_DISTANCE == maxRange){
			tilesToCheck.pop();
			continue;
		}
		...
		if( NORTH > 0 
		and not m_validActionDistance.count(NORTH)) {
			m_validActionDistance[NORTH] = CURRENT_DISTANCE + 1;
			tilesToCheck.push(NORTH);
		}

		if( EAST / boardLine == tilesToCheck.front() / boardLine
		and not m_validActionDistance.count(EAST)) {
			m_validActionDistance[EAST] = CURRENT_DISTANCE + 1;
			tilesToCheck.push(EAST);
		}

		if( SOUTH <= lastTile
		and not m_validActionDistance.count(SOUTH)) {
			m_validActionDistance[SOUTH] = CURRENT_DISTANCE + 1;
			tilesToCheck.push(SOUTH);
		}

		if( WEST / boardLine == tilesToCheck.front() / boardLine
		and WEST >= 0
		and not m_validActionDistance.count(WEST)) {
			m_validActionDistance[WEST] = CURRENT_DISTANCE + 1;
			tilesToCheck.push(WEST);
		}
	...
	// Remove tiles closer than min range
	auto iter 	= m_validActionDistance.begin();
	auto endIter	= m_validActionDistance.end();
	while(iter != endIter){
		if(iter->second < minRange) iter = m_validActionDistance.erase(iter);	
		else iter++;
	}

	for(auto const& [tile, distance] : m_validActionDistance){
		vec2D tile2D = {tile % boardLine, tile / boardLine};
		m_validAction.push_back(tile2D);
	}
...
// Draw action range
void cUnitRange::drawActionRange(vec2D p_cameraOffset, int p_animationFrame){
	SDL_Rect srcRect{0, 0, TILE_SIZE, TILE_SIZE};
	SDL_Rect dstRect{0, 0, TILE_SIZE, TILE_SIZE};
	...
	//Draw range tiles to screen 
	for(size_t i = 0; i < m_validAction.size(); i++){
		dstRect.x = m_validAction[i].x * TILE_SIZE + p_cameraOffset.x;
		dstRect.y = m_validAction[i].y * TILE_SIZE + p_cameraOffset.y;
		SDL_RenderCopy(g_renderer, m_actionRangeSprite, &srcRect, &dstRect);
	}
}
